...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="第一步输出的用户特征文件路径，留空则自动根据 ratings-path 推断",
    )
    parser.add_argument(
        "--gnn-items-path",
        type=str,
        default="",
        help="第一步输出的电影特征文件路径，留空则自动根据 ratings-path 推断",
    )
    parser.add_argument(
        "--gnn-model-list",
        type=str,
        default=",".join(默认GNN模型列表),
        help="图模型列表，多个值用英文逗号分隔，可选 gcn, graphsage",
    )
    parser.add_argument(
        "--gnn-hidden-dim-list",
        type=str,
        default=",".join(map(str, 默认GNN隐藏维度列表)),
        help="图模型隐藏维度列表，多个值用英文逗号分隔",
    )
    parser.add_argument(
        "--gnn-layers-list",
        type=str,
        default=",".join(map(str, 默认GNN层数列表)),
        help="图模型层数列表，多个值用英文逗号分隔",
    )
    parser.add_argument(
        "--gnn-lr-list",
        type=str,
        default=",".join(map(str, 默认GNN学习率列表)),
        help="图模型学习率列表，多个值用英文逗号分隔",
    )
    parser.add_argument(
        "--gnn-weight-decay-list",
        type=str,
        default=",".join(map(str, 默认GNN权重衰减列表)),
        help="图模型权重衰减列表，多个值用英文逗号分隔",
    )
    parser.add_argument(
        "--gnn-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=默认GNN设备,
        help="图模型运行设备，留空则自动选择 cuda 或 cpu",
    )
    ...
    itemcf_sim_list = 解析字符串列表(args.itemcf_sim_list)
    ...
    gnn_model_list = [x.lower() for x in 解析字符串列表(args.gnn_model_list)]
    gnn_hidden_dim_list = 解析整数列表(args.gnn_hidden_dim_list)
    gnn_layers_list = 解析整数列表(args.gnn_layers_list)
    gnn_lr_list = 解析浮点列表(args.gnn_lr_list)
    gnn_weight_decay_list = 解析浮点列表(args.gnn_weight_decay_list)
    gnn_device = args.gnn_device.strip() if args.gnn_device.strip() else None
    ...
    gnn_results, _ = sweep_gnn_feature(
        ...
        model_types=gnn_model_list,
        hidden_dims=gnn_hidden_dim_list,
        num_layers_list=gnn_layers_list,
        lr_list=gnn_lr_list,
        weight_decay_list=gnn_weight_decay_list,
